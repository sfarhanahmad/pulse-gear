...
function HomePage() {
  ...
  useEffect(() => {
    setProducts(getProducts()); setContent(getSiteContent())
    function onUpdate() { setProducts(getProducts()); setContent(getSiteContent()); setCategories(getCategories()) }
    window.addEventListener('pg_products_updated', onUpdate)
    window.addEventListener('pg_categories_updated', onUpdate)
    window.addEventListener('pg_content_updated', onUpdate)
    return () => { window.removeEventListener('pg_products_updated', onUpdate)
      window.removeEventListener('pg_categories_updated', onUpdate); window.removeEventListener('pg_content_updated', onUpdate) }
  }, [])
  ...
  return (
    <main>
      {/* Hero */}
      <section style={{
        position: 'relative', minHeight: '88vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
        ...(content.heroImage ? { backgroundImage: `url(${content.heroImage})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {})
      }}>
        ...
        <div style={{ position: 'absolute', bottom: '15%', left: '5%', width: 200, height: 200, borderRadius: '50%', background: 'var(--accent-subtle)', filter: 'blur(40px)', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', padding: '0 1rem', maxWidth: 900, margin: '0 auto' }} className="animate-fade-up">
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="#products" style={{ background: 'var(--accent)', color: 'var(--accent-text)', fontWeight: 700, fontSize: '1rem', padding: '14px 32px', borderRadius: 14, textDecoration: 'none', fontFamily: 'var(--font-display)', transition: 'all 0.2s', boxShadow: '0 4px 20px var(--accent-subtle)' }}
            ...
            <a href={waLink} target="_blank" rel="noopener noreferrer"
              style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)', color: 'var(--text)', fontWeight: 600, fontSize: '1rem', padding: '14px 32px', borderRadius: 14, textDecoration: 'none', transition: 'all 0.2s' }}
      ...
      <section id="products" style={{ maxWidth: 1152, margin: '0 auto', padding: '4rem 1rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 40 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {[{ id: 'all' as const, label: 'All', icon: '🛍️' }, ...usedCategories].map(cat => {
              const isActive = activeCategory === cat.id
              return (
                <button key={cat.id} onClick={() => setActiveCategory(cat.id as any)}
                  style={{
                    padding: '8px 18px', borderRadius: 99, fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'var(--font-body)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <select value={sortBy} onChange={e => setSortBy(e.target.value as SortOption)} className="pg-input" style={{ width: 'auto', padding: '8px 14px', borderRadius: 10, fontSize: '0.85rem' }}>
      ...
      <section style={{ maxWidth: 1152, margin: '0 auto', padding: '0 1rem 5rem' }}>
        <div style={{ background: 'var(--accent)', borderRadius: 24, padding: '4rem 2rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', bottom: -30, left: -30, width: 150, height: 150, borderRadius: '50%', background: 'rgba(0,0,0,0.06)' }} />
          ...
          <a href={`${waLink}?text=Hi!%20I'd%20like%20to%20browse%20your%20products.`} target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#000', color: '#fff', fontWeight: 700, fontSize: '1rem', padding: '14px 32px', borderRadius: 14, textDecoration: 'none', position: 'relative', transition: 'all 0.2s' }}
            onMouseEnter={e => { (e.target as HTMLElement).style.background = '#222' }}
            onMouseLeave={e => { (e.target as HTMLElement).style.background = '#000' }}>
            💬 Order on WhatsApp
          </a>
        </div>
      </section>
...
function ProductCard({ product, waNumber, categories }: { product: Product; waNumber: string; categories: ReturnType<typeof getCategories> }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '20px 20px 24px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', lineHeight: 1.6, flex: 1, margin: '0 0 20px' }}>{product.shortDescription}</p>
