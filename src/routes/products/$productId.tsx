  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="flex flex-col md:flex-row gap-12 items-start">
        ...
        <div className="w-full md:w-1/2">
          <span className="text-yellow-400 text-sm font-semibold uppercase tracking-widest">{catLabel}</span>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-3xl font-black text-yellow-400">Rs. {product.price.toLocaleString()}</span>
            </div>
          </div>
          <WhatsAppButton productName={product.name} waNumber={waNumber} className="w-full justify-center text-lg py-4" />
          <p className="text-white/30 text-sm text-center mt-4">Fast delivery · Secure WhatsApp ordering · Trusted seller</p>
        </div>
      </div>
    </main>
  )
