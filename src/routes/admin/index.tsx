      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {products.map(product => (
          <div key={product.id} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-yellow-400/30 transition-all">
            ...
            <div className="p-4">
              <span className="text-yellow-400 text-xs font-semibold uppercase">{['earbuds','headphones'].includes(product.category) ? product.category === 'earbuds' ? 'Earbuds' : 'Headphones' : product.category === 'watch' ? 'Watch' : product.category === 'wallet' ? 'Wallet' : 'Power Bank'}</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Category *">
              <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value as 'earbuds' | 'watch' }))} className={inputCls}>
                <option value="earbuds">Ear Pods & Earbuds</option>
          ...
          {products.map(product => (
            <div key={product.id} className="flex items-center gap-4 bg-white/5 rounded-xl p-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white/40 text-xs mt-0.5">{['earbuds','headphones'].includes(product.category) ? product.category === 'earbuds' ? 'Earbuds' : 'Headphones' : product.category === 'watch' ? 'Watch' : product.category === 'wallet' ? 'Wallet' : 'Power Bank'}</p>
              </div>
              <div>
