  ...
  return (
    <main>
      ...
      <section style={{ position: 'relative', minHeight: '92vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', padding: '0 1rem', maxWidth: 900, margin: '0 auto', animation: 'fadeUp 0.6s cubic-bezier(0.16,1,0.3,1) both' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'var(--accent-subtle)', border: '1px solid var(--accent-border)', borderRadius: 99, padding: '6px 18px', marginBottom: 32, animation: 'pulse-glow 2s ease-in-out infinite' }}>
      ...
      <section style={{ borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', background: 'var(--bg-subtle)' }}>
        <div style={{ maxWidth: 1152, margin: '0 auto', padding: '24px 1rem', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, textAlign: 'center' }}>
          {content.stats.map((stat, i) => (
            <div key={stat.label} style={{ animation: `fadeUp 0.6s cubic-bezier(0.16,1,0.3,1) ${i * 0.1}s both` }}>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.6rem', background: 'linear-gradient(135deg, #a78bfa, #c4b5fd)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{stat.value}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 16 }}>
          {usedCategories.map((cat, i) => (
            <div key={cat.id} className="cat-card"
              style={{ height: 200, animation: `fadeUp 0.6s cubic-bezier(0.16,1,0.3,1) ${i * 0.1}s both` }}
              onClick={() => { setActiveCategory(cat.id); document.getElementById('products')?.scrollIntoView({ behavior: 'smooth' }) }}>
              <img
                src={(cat as any).image || CATEGORY_IMAGES[cat.id] || 'https://images.unsplash.com/photo-1518770660439-4636190af475?w=600&q=80'}
                alt={cat.label}
      ...
      <section id="products" style={{ maxWidth: 1152, margin: '0 auto', padding: '2rem 1rem' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 40, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '14px 18px', animation: 'fadeUp 0.6s cubic-bezier(0.16,1,0.3,1) 0.2s both' }}>
        ...
        {activeCategory === 'all' && grouped ? (
          grouped.map(cat => cat.products.length > 0 && (
            <div key={cat.id} style={{ marginBottom: '4rem' }}>
              <div style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'baseline', gap: 12, animation: 'fadeUp 0.6s cubic-bezier(0.16,1,0.3,1) both' }}>
      ...
      {/* CTA */}
      <section style={{ maxWidth: 1152, margin: '0 auto', padding: '0 1rem 5rem', animation: 'fadeUp 0.6s cubic-bezier(0.16,1,0.3,1) 0.3s both' }}>
        <div style={{ background: 'linear-gradient(135deg, var(--accent) 0%, var(--accent2) 50%, #6d28d9 100%)', borderRadius: 24, padding: '4rem 2rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <a href={`${waLink}?text=Hi!%20I'd%20like%20to%20browse%20your%20products.`} target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#fff', color: 'var(--accent)', fontWeight: 700, fontSize: '1rem', padding: '14px 32px', borderRadius: 14, textDecoration: 'none', position: 'relative', fontFamily: 'var(--font-display)', transition: 'all 0.2s' }}
...
function ProductCard({ product, waNumber, categories, index }: { product: Product; waNumber: string; categories: Category[]; index: number }) {
  ...
  return (
    <div className="product-card" style={{ animation: `fadeUp 0.6s cubic-bezier(0.16,1,0.3,1) ${index * 0.08}s both` }}>
      ...
      <div style={{ padding: '20px 20px 24px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handleAddToCart} style={{
            flexShrink: 0,
            background: added ? 'linear-gradient(135deg, var(--accent), var(--accent2))' : 'var(--accent-subtle)',
            border: '1px solid var(--accent-border)',
            color: added ? '#fff' : 'var(--accent2)',
