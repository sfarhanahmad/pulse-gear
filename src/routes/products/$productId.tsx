...
function RouteComponent() {
  ...
  useEffect(() => {
    async function load() {
      const [products, content, cats] = await Promise.all([getProducts(), getSiteContent(), getCategories()])
    ...
    <main style={{ maxWidth: 1152, margin: '0 auto', padding: '2rem 1rem 5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', alignItems: 'start' }}>
        ...
        <div>
          ...
          <p style={{ color: 'var(--text-faint)', fontSize: '0.8rem', textAlign: 'center', marginTop: 16 }}>⚡ Fast delivery · 🔒 Secure ordering · ✅ Trusted seller</p>
        </div>
      </div>
    </main>
  )
