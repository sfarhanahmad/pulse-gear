  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setAdding(true)} className="bg-yellow-400 hover:bg-yellow-300 text-black font-bold px-5 py-2.5 rounded-xl text-sm transition-colors">+ Add Product</button>
      </div>
...
function ProductModal({ product, onClose, onSave }: { product: Product | null; onClose: () => void; onSave: () => void }) {
  ...
  const [form, setForm] = useState({ name: product?.name ?? '', category: (product?.category ?? 'earbuds') as 'earbuds' | 'watch', price: product?.price?.toString() ?? '', shortDescription: product?.shortDescription ?? '', description: product?.description ?? '', image: product?.image ?? '/placeholder.png' })
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-4 py-8 overflow-y-auto">
      <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-2xl">
        ...
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          <div>
            ...
            <div className="flex items-start gap-4">
              <div className="w-28 h-28 rounded-xl bg-white/5 border border-white/10 overflow-hidden flex-shrink-0">
                {imagePreview ? <img src={imagePreview} alt="Preview" className="w-full h-full object-cover" onError={e => { (e.target as HTMLImageElement).src = '/placeholder.png' }} /> : <div className="w-full h-full flex items-center justify-center text-white/20 text-xs">No image</div>}
              </div>
              <div className="flex-1">
                <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
                <button onClick={() => fileRef.current?.click()} disabled={uploading} className="bg-white/5 hover:bg-white/10 border border-white/10 text-white text-sm font-semibold px-4 py-2.5 rounded-xl w-full">{uploading ? 'Uploading...' : '📁 Upload Image'}</button>
                <p className="text-white/30 text-xs mt-2">Or enter URL:</p>
                <input type="text" value={form.image.startsWith('data:') ? '' : form.image} onChange={e => { setForm(f => ({ ...f, image: e.target.value })); setImagePreview(e.target.value) }} placeholder="https://..." className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm placeholder-white/20 focus:outline-none mt-1" />
              </div>
            </div>
          </div>
          ...
          <Field label="Short Description *"><input type="text" required value={form.shortDescription} onChange={e => setForm(f => ({ ...f, shortDescription: e.target.value }))} placeholder="One-line summary" className={inputCls} /></Field>
        ...
        <div className="flex gap-3 p-6 border-t border-white/10">
          ...
          <button onClick={handleSave} disabled={!form.name || !form.price || !form.description || !form.shortDescription} className="flex-1 bg-yellow-400 hover:bg-yellow-300 disabled:opacity-50 text-black font-bold py-3 rounded-xl">{isEdit ? 'Save Changes' : 'Add Product'}</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleSave} className={`font-bold px-6 py-2.5 rounded-xl text-sm transition-all ${saved ? 'bg-green-400 text-black' : 'bg-yellow-400 hover:bg-yellow-300 text-black'}`}>{saved ? '✓ Saved!' : 'Save Changes'}</button>
      </div>
      <div className="flex gap-2 mb-6 flex-wrap">
        {(['hero', 'stats', 'cta', 'about'] as const).map(s => (
          <button key={s} onClick={() => setSection(s)} className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${section === s ? 'bg-yellow-400/20 border border-yellow-400/40 text-yellow-400' : 'bg-white/5 text-white/50 hover:text-white'}`}>{s.toUpperCase()}</button>
        ))}
      </div>
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-5">
        {section === 'hero' && (<>
          <Field label="Hero Tagline"><input value={form.heroTagline} onChange={e => setForm(f => ({ ...f, heroTagline: e.target.value }))} className={inputCls} /></Field>
          <Field label="Hero Title"><textarea rows={2} value={form.heroTitle} onChange={e => setForm(f => ({ ...f, heroTitle: e.target.value }))} className={`${inputCls} resize-none`} /></Field>
          ...
          <Field label="WhatsApp Number (with country code)"><input value={form.whatsappNumber} onChange={e => setForm(f => ({ ...f, whatsappNumber: e.target.value }))} className={inputCls} placeholder="923266570023" /></Field>
        </>)}
        {section === 'stats' && form.stats.map((stat, i) => (
          <div key={i} className="grid grid-cols-2 gap-3 bg-white/5 rounded-xl p-4">
            <Field label={`Stat ${i + 1} Value`}><input value={stat.value} onChange={e => updateStat(i, 'value', e.target.value)} className={inputCls} /></Field>
            <Field label="Label"><input value={stat.label} onChange={e => updateStat(i, 'label', e.target.value)} className={inputCls} /></Field>
          </div>
        ))}
        {section === 'cta' && (<>
          <Field label="CTA Title"><input value={form.ctaBannerTitle} onChange={e => setForm(f => ({ ...f, ctaBannerTitle: e.target.value }))} className={inputCls} /></Field>
          <Field label="CTA Subtitle"><input value={form.ctaBannerSubtitle} onChange={e => setForm(f => ({ ...f, ctaBannerSubtitle: e.target.value }))} className={inputCls} /></Field>
        </>)}
        {section === 'about' && (<>
          {form.aboutStory.map((para, i) => <Field key={i} label={`Paragraph ${i + 1}`}><textarea rows={3} value={para} onChange={e => updateAboutStory(i, e.target.value)} className={`${inputCls} resize-none`} /></Field>)}
          {form.aboutValues.map((v, i) => (
            <div key={i} className="grid grid-cols-3 gap-3 bg-white/5 rounded-xl p-4">
              <Field label="Icon"><input value={v.icon} onChange={e => updateAboutValue(i, 'icon', e.target.value)} className={inputCls} /></Field>
              <Field label="Title"><input value={v.title} onChange={e => updateAboutValue(i, 'title', e.target.value)} className={inputCls} /></Field>
              <Field label="Desc"><input value={v.desc} onChange={e => updateAboutValue(i, 'desc', e.target.value)} className={inputCls} /></Field>
            </div>
          ))}
        </>)}
      </div>
...
function ImagesTab({ products, onRefresh }: { products: Product[]; onRefresh: () => void }) {
  ...
  return (
    <div>
      {toast && <div className="fixed bottom-6 right-6 bg-yellow-400 text-black font-bold px-5 py-3 rounded-xl shadow-lg z-50">✓ {toast}</div>}
      <div className="mb-6"><h2 className="text-2xl font-black">Image Manager</h2><p className="text-white/40 text-sm mt-1">Upload and replace website images</p></div>
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-6">
        ...
        <div className="flex items-start gap-4 mt-4">
          <div className="w-40 h-24 rounded-xl bg-white/5 border border-white/10 overflow-hidden flex-shrink-0">
            {heroImage ? <img src={heroImage} alt="Hero" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-white/20 text-xs text-center p-2">No hero image</div>}
          </div>
          <div>
            <input ref={heroFileRef} type="file" accept="image/*" className="hidden" onChange={handleHeroUpload} />
            <button onClick={() => heroFileRef.current?.click()} className="bg-yellow-400 hover:bg-yellow-300 text-black font-bold px-5 py-2.5 rounded-xl text-sm">📁 Upload Hero Image</button>
            {heroImage && <button onClick={() => { saveSiteContent({ heroImage: '' }); saveImage('hero_bg', ''); setHeroImage(''); showToast('Removed.') }} className="ml-2 bg-red-500/10 text-red-400 text-sm font-semibold px-3 py-2.5 rounded-xl">Remove</button>}
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {products.map(product => (
            <div key={product.id} className="flex items-center gap-4 bg-white/5 rounded-xl p-4">
              ...
              <div className="flex-1 min-w-0"><p className="font-semibold text-sm truncate">{product.name}</p></div>
              <div>
                <input ref={el => { productFileRefs.current[product.id] = el }} type="file" accept="image/*" className="hidden" onChange={e => handleProductImageUpload(product.id, e)} />
                <button onClick={() => productFileRefs.current[product.id]?.click()} className="bg-white/5 hover:bg-white/10 border border-white/10 text-white text-xs font-semibold px-3 py-2 rounded-lg">Change Image</button>
              </div>
            </div>
          ))}
        </div>
